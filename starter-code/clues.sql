...
-- Hint: first find the countries where only one language is spoken. Then, in a separate query, find the countries where Italian is spoken. Then cross reference these lists by hand.
 ...
 SELECT code, name
 FROM countries
 INNER JOIN countrylanguages ON countries.code = countrylanguages.countrycode
...
FROM cities
INNER JOIN countries ON cities.countrycode = countries.code AND countries.capital = cities.id
...
SELECT id, name, countrycode FROM cities WHERE population = 91084;

-- She's in ____SANTA MONICA______!
